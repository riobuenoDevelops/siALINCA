import pdfMake from "pdfmake/build/pdfmake";
import pdfFonts from "pdfmake/build/vfs_fonts";
pdfMake.vfs = pdfFonts.pdfMake.vfs;

var datetime = new Date();

function crearNotaPDF(){
  var info = {Lugar: 'lugar', Solicitante: 'nombre', Cedula: 'cedula'}
  //var info = arguments[0];
  //var data = arguments[1];
  var notaDocument={
    info:{
      title: 'Nota de Entrega '+datetime.getDate()+'/'+(datetime.getMonth() + 1)+'/'+datetime.getFullYear(),
      author: 'ALINCA'
    },

    styles:{
      tablas:{
        fontSize: 9,
        margin: [0,0,0,0]
      },
      firma:{
        margin: [30,10]
      }
    },

    content:[
      {
        image: 'data:image/png;base64,'
      },
      {
        text: 'NOTA DE ENTREGA DE SALIDA DE INSUMOS Y/O MATERIALES REQUERIDOS POR JORNADAS',
        bold: true,
        alignment: 'center',
        margin: [30,8],
        fontSize: 14
      },
      {
        text: 'LUGAR O CENTRO DE ATENCION: '+info.Lugar,
        margin: [0,10,0,0],
        fontSize: 10,
        bold: true,
        alignment: 'left'
      },
      {
        columns:[
          {
            text: 'SOLICITANTE: '+info.Solicitante,
            fontSize: 10,
            bold: true,
            alignment: 'left'
          },
          {
            text: 'CEDULA: '+info.Cedula,
            fontSize: 10,
            bold: true,
            alignment: 'center'
          },
          {
            text: 'FECHA: '+datetime.getDate()+'/'+(datetime.getMonth() + 1)+'/'+datetime.getFullYear(),
            fontSize: 10,
            bold: true,
            alignment: 'right'
          },
        ],
        margin: [0,6,0,10],
        alignment: 'left'
      },
      {
        style: 'tablas',
        table:{
          widths:[25,40,245,'auto','auto','auto','auto'],
          body:[
            [
                {text: 'N°', rowSpan: 2, alignment: 'center', bold: true, margin:[0,8,0,0]},
                {text: 'Tipo', rowSpan: 2, alignment: 'center', bold: true, margin: [0,8,0,0]}, 
                {text: 'Descripcion del Material', rowSpan: 2, alignment: 'center', bold: true, margin:[0,8,0,0]},
                {text: 'Retirados',colSpan: 2, alignment: 'center', bold: true},
                '',
                {text: 'Devueltos', colSpan: 2, alignment: 'center', bold: true},
                ''
            ],
            [
                '',
                '',
                '',
                'Unidad',
                'Cantidad',
                'Unidad',
                'Cantidad'
                
            ]
          ]
        }
      },
      {
        style: 'firma',
        table:{
          widths:[100,100,100,100],
          body: [
            [
              {text:'SALIDA', colSpan: 2, alignment: 'center', bold: true, fontSize: 10, fillColor: '#cccccc'},
              '',
              {text:'ENTRADA', colSpan: 2, alignment: 'center', bold: true, fontSize: 10, fillColor: '#cccccc'},
              ''
            ],
            [
              {text:'ENTREGADO POR\n\n\n\n______________________\nNombre y Apellido\n\n\n\n______________________\nFirma', 
                alignment: 'center', bold: true, fontSize: 8},
              {text:'RECIBIDO POR\n\n\n\n______________________\nNombre y Apellido\n\n\n\n______________________\nFirma', 
                alignment: 'center', bold: true, fontSize: 8},
              {text:'ENTREGADO POR\n\n\n\n______________________\nNombre y Apellido\n\n\n\n______________________\nFirma', 
                alignment: 'center', bold: true, fontSize: 8},
              {text:'RECIBIDO POR\n\n\n\n______________________\nNombre y Apellido\n\n\n\n______________________\nFirma', 
                alignment: 'center', bold: true, fontSize: 8}
            ]
          ]
        }
      }
    ]
  }

  //Agregar contenido a la tabla con este Path de la tabla
  //La funcion puede recibir como parametro una lista con el orden indicado
  //info = {Lugar: 'lugar', Solicitante: 'nombre', Cedula: 'cedula'}
  //data = [[Tipo, Descripcion(Nombre), Retirado.Unidad, Retirado.Cantidad, '', '']]
  var data = [['MDC','cosa','1','2','1','2'],['MDC','cosasdasd','1','2','1','2'],['MDC','cosasdasd','1','32','24','124']];
  ////////////////////////////////////////////////////////////////////////////////////////
  var number = [1];
  data.forEach(insertar);
  function insertar(value){
    var v = number.concat(value);
    notaDocument.content[4].table.body.push(v);
    number[0]++;
  }

  pdfMake.createPdf(notaDocument).open();
}

function crearInventarioPDF(){
  //var data = arguments[0];

  var inventarioDocument = {
    info:{
      title: 'Inventario '+datetime.getDate()+'/'+(datetime.getMonth() + 1)+'/'+datetime.getFullYear(),
      author: 'ALINCA'
    },

    styles:{
      tablas:{
        fontSize: 9,
        margin: [0,0,0,0]
      }
    },

    content:[
      {
        image: 'data:image/png;base64,'
      },
      {
        text: 'INVENTARIO',                             
        bold: true,
        alignment: 'center',
        margin: [0,2,0,4]
      },
      {
        text: 'FECHA: '+datetime.getDate()+'/'+(datetime.getMonth() + 1)+'/'+datetime.getFullYear(),
        bold: true,
        alignment: 'right',
        margin: [0,0,18,10],
        fontSize: 9
      },
      {
        style: 'tablas',
        table:{
          widths:[40,260,60,100],
          body:[
            [
              {text: 'Tipo', alignment: 'center', bold: true, margin: [0,4]},
              {text: 'Descripcion del Material', alignment: 'center', bold: true, margin:[0,4]},
              {text: 'Cantidad', alignment: 'center', bold: true, margin:[0,4]},
              {text: 'Fecha de Vencimiento', alignment: 'center', bold: true, margin:[0,4]}
            ]          
          ]
        }
      }
    ]
  }
  var data = [['MDC','cosa','1','Algun dia'],['MDC','cosasdasd','1','2'],['MDC','cosasdasd','1','32']];

  data.forEach(insertar);
  function insertar(value){
    inventarioDocument.content[3].table.body.push(value);
  }

  pdfMake.createPdf(inventarioDocument).open();
}

function crearOrdenCompraPDF(){
  //var data = arguments[0];
  var data = {
    id: '5545161',
    divisa: 'dolar',
    facturas: [['numero factura','nombre comercio'], ['factura 2','comercio 2']],
    articulos: [['nombre item',5,90]],
    costoTotal: 1500
  };//Borrar esta variable 'data' al habilitar el de data = arguments[0]
  
  var ordenCompraDocument={
    info:{
      title: 'Orden de Compra '+datetime.getDate()+'/'+(datetime.getMonth() + 1)+'/'+datetime.getFullYear(),
      author: 'ALINCA'
    },

    styles:{
      tablas:{
        fontSize: 9,
        margin: [0,0,0,0]
      }
    },

    content:[
      {
        image: 'data:image/png;base64,'
      },
      {
        text: 'ORDEN DE COMPRA',                             
        bold: true,
        alignment: 'center',
        margin: [0,4,0,8]
      },
      {
        columns:[
          {
            text: 'Codigo: '+data.id,
            fontSize: 10,
            bold: true,
            width: '25%',
            alignment: 'left'
          },
          {
            text: 'Divisa: '+data.divisa,
            fontSize: 10,
            bold: true,
            width: '25%'
          },
          {
            text: 'Fecha: '+datetime.getDate()+'/'+(datetime.getMonth() + 1)+'/'+datetime.getFullYear(),
            fontSize: 10,
            bold: true,
            width: '*',
            alignment: 'right'
          }
        ],
        margin: [20,15,20,8],
        alignment: 'center'
      },
      {
        text: '_____________________________________________________________________________________________',
        alignment: 'center',
        size: 12,
        margin: [0,0,0,0]
      },
      {
        text: 'Facturas',                             
        bold: true,
        alignment: 'center',
        size: 10,
        margin: [0,5,0,5]
      },
      {
        style: 'tablas',
        margin: [80,0],
        table: {
          widths: [100,250],
          body: [
            [
              {
                text: 'Numero de Factura', alignment: 'center', bold: true, fillColor: '#cccccc'
              },
              {
                text: 'Comercio', alignment: 'center', bold: true, fillColor: '#cccccc'
              }
            ]
          ]
        }
      },
      {
        style: 'tablas',
        margin: [20,20],
        table: {
          widths: [200,50,80,100],
          body: [
            [
              {
                text: 'Articulo', alignment: 'center', bold: true, fillColor: '#cccccc'
              },
              {
                text: 'Cantidad', alignment: 'center', bold: true, fillColor: '#cccccc'
              },
              {
                text: 'Precio Unitario', alignment: 'center', bold: true, fillColor: '#cccccc'
              },
              {
                text: 'Precio Total', alignment: 'center', bold: true, fillColor: '#cccccc'
              },
            ]
          ]
        }
      },
    ]
  }

  data.facturas.forEach(insertar, 5);
  data.articulos.forEach(insertar, 6);
  ordenCompraDocument.content[6].table.body.push([{
    text:'', border:[false,true,false,false]
  },
  {
    text:'', border:[false,true,true,false]
  },
  {
    text:'Costo Total',
    alignment:'center',
    bold:true,
    fillColor: '#cccccc'
  },
  data.costoTotal]);

  function insertar(value){
    if (this == 6){
      var precioTotal = [value[1]*value[2]];
      var v = value.concat(precioTotal);
      ordenCompraDocument.content[this].table.body.push(v);
      return;
    }
    ordenCompraDocument.content[this].table.body.push(value);
  }

  //Agregar facturas en la tabla, usar data.facturas.forEach(insertar, 6)
  //ordenCompraDocument.content[this].table.body.push(facturas);

  //Agregar los articulos en la tabla con, usar data.articulos.forEach(insertar, 7)
  //ordenCompraDocument.content[this].table.body.push(articulos);
  //Luego agregar el Costo Total afuera del forEach
  //ordenCompraDocument.content[7].table.body.push(['','',{text:'Costo Total',alignment:'center,bold:true,fillColor: '#cccccc'},data.totalPrice]);

  /*ordenCompra = {
    id: purchaseOrderId,
    divisa: currency,
    facturas: [['numero factura','nombre comercio'], ['factura 2','comercio 2']],
    articulos: [['nombre item','quantity','price'],[otro articulo con ese formato]],
    costoTotal: totalPrice
  }*/

  pdfMake.createPdf(ordenCompraDocument).open();
}

function crearReportePDF(){
  //var data = arguments[0];
  var data = {
    reporteRango: {
      desde: 'fechaDesde',
      hasta: 'fechaHasta',
      tipo: 'semanal/mensual/etc'
    },
    reporteResponsable: 'Luis Saavedra',
    ordenCompraCantidad: 20,
    itemsAgregados: [['Adc','Cartera',4,['san felix','narnia'],'05/07/2030'],['Ad','Caera',2,['san fx','naia'],'23/17/2230']],
    almacenes: [['alm','almacen1','cartera',4,'05/07/2030']],
    notaNoRetorno: [['ENT1','luis fernando','despachador',16,'09/09/9999']],
    notaConRetorno: [['ENT4','luis fernando','despachador',50,'09/09/9999','08/08/8888']]
  };//Borrar esta variable 'data' al habilitar el de data = arguments[0]

  var reporteDocument = {

    info:{
      title: 'Reporte '+datetime.getDate()+'/'+(datetime.getMonth() + 1)+'/'+datetime.getFullYear(),
      author: 'ALINCA'
    },

    styles:{
      tablas:{
        fontSize: 9,
        margin: [0,0,0,12]
      }
    },

    content: [
      {
        image: 'data:image/png;base64,'
      },
      {
        text: 'Reporte '+data.reporteRango.tipo,
        alignment: 'center',
        bold: 'true',
        margin: [0,8,0,0]
      },
      {
        text: data.reporteRango.desde+' - '+data.reporteRango.hasta,
        alignment: 'center',
        bold: 'true',
        margin: [0,1,0,15]
      },
      {
        columns:[
          {
            text: [
              {text: 'Reporte Generado por: ', bold: true},
              data.reporteResponsable
            ],
            fontSize: 10,
            width: '70%',
            alignment: 'left'
          },
          {
            text: [
              {text: 'Fecha Generado: ', bold: true},
              datetime.getDate()+'/'+(datetime.getMonth() + 1)+'/'+datetime.getFullYear()
            ],
            fontSize: 10,
            width: '30%',
            alignment: 'right'
          }
        ]
      },
      '_______________________________________________________________________________________________\n ',
      {
        fontSize: 10,
        ul: [
          {
            text: [
              {text: 'Ordenes de Compra Realizadas: ', bold: true},
              data.ordenCompraCantidad+'\n '
            ]
          },
          {
            text: 'Items Agregados\n ',
            bold: true
          }
        ]
      },
      {//6
        style: 'tablas',
        table: {
          widths: [54,120,40,170,75],
          body: [
            [
              {
                text: 'Codigo', alignment: 'center', bold: true, fillColor: '#cccccc', margin: [0,2]
              },
              {
                text: 'Nombre', alignment: 'center', bold: true, fillColor: '#cccccc', margin: [0,2]
              },
              {
                text: 'Cantidad', alignment: 'center', bold: true, fillColor: '#cccccc', margin: [0,2]
              },
              {
                text: 'Ubicacion en Almacenes', alignment: 'center', bold: true, fillColor: '#cccccc', margin: [0,2]
              },
              {
                text: 'Fecha Agregado', alignment: 'center', bold: true, fillColor: '#cccccc', margin: [0,2]
              }
            ],
          ]
        }
      },
      {
        fontSize: 10,
        bold: true,
        ul: [
          'Almacenes\n '
        ]
      },
      {//8
        style: 'tablas',
        margin: [10,0,0,12],
        table: {
          widths: [54,120,140,50,75],
          body: [
            [
              {
                text: 'Codigo', alignment: 'center', bold: true, fillColor: '#cccccc', margin: [0,2]
              },
              {
                text: 'Nombre', alignment: 'center', bold: true, fillColor: '#cccccc', margin: [0,2]
              },
              {
                text: 'Item Agregado', alignment: 'center', bold: true, fillColor: '#cccccc', margin: [0,2]
              },
              {
                text: 'Cantidad', alignment: 'center', bold: true, fillColor: '#cccccc', margin: [0,2]
              },
              {
                text: 'Fecha Agregado', alignment: 'center', bold: true, fillColor: '#cccccc', margin: [0,2]
              }
            ]
          ]
        }
      },
      {
        fontSize: 10,
        bold: true,
        ul: [
          'Notas de Entrega sin Retorno\n '
        ]
      },
      {//10
        style: 'tablas',
        margin: [10,0,0,12],
        table: {
          widths: [54,120,120,70,75],
          body: [
            [
              {
                text: 'Codigo', alignment: 'center', bold: true, fillColor: '#cccccc', margin: [0,5]
              },
              {
                text: 'Responsable', alignment: 'center', bold: true, fillColor: '#cccccc', margin: [0,5]
              },
              {
                text: 'Despachado Por', alignment: 'center', bold: true, fillColor: '#cccccc', margin: [0,5]
              },
              {
                text: 'Cant.\n Items Pedidos', alignment: 'center', bold: true, fillColor: '#cccccc'
              },
              {
                text: 'Fecha Realizado', alignment: 'center', bold: true, fillColor: '#cccccc', margin: [0,5]
              }
            ]
          ]
        }
      },
      {
        fontSize: 10,
        bold: true,
        ul: [
          'Notas de Entrega con Retorno\n '
        ]
      },
      {//12
        style: 'tablas',
        table: {
          widths: [54,120,120,70,'auto','auto'],
          body: [
            [
              {
                text: 'Codigo', rowSpan: 2, alignment: 'center', bold: true, fillColor: '#cccccc', margin: [0,11]
              },
              {
                text: 'Responsable', rowSpan: 2, alignment: 'center', bold: true, fillColor: '#cccccc', margin: [0,11]
              },
              {
                text: 'Despachado Por', rowSpan: 2, alignment: 'center', bold: true, fillColor: '#cccccc', margin: [0,11]
              },
              {
                text: 'Cant.\n Items Pedidos', rowSpan: 2, alignment: 'center', bold: true, fillColor: '#cccccc', margin: [0,6]
              },
              {
                text: 'Fecha', colSpan: 2, alignment: 'center', bold: true, fillColor: '#cccccc', margin: [0,2]
              },
              ''
            ],
            [
              '',
              '',
              '',
              '',
              {
                text: 'Entrega', alignment: 'center', bold: true, fillColor: '#cccccc', margin: [3,2]
              },
              {
                text: 'Retorno', alignment: 'center', bold: true, fillColor: '#cccccc', margin: [3,2]
              }
            ]
          ]
        }
      },
    ]
  }

  data.itemsAgregados.forEach(insertar,6);
  data.almacenes.forEach(insertar, 8);
  data.notaNoRetorno.forEach(insertar, 10);
  data.notaConRetorno.forEach(insertar, 12);

  function insertar(value){
    if (this == 6){
      var cadena = value[3].shift();
      if (value[3].length != 0){
        var cantidad = value[3].length;
        for (var i=0; i < cantidad; i++)
          cadena = cadena.concat(' - '+value[3].shift());
      }
      value[3] = cadena;
      reporteDocument.content[this].table.body.push(value);
      return;
    }
    reporteDocument.content[this].table.body.push(value);
  }

  pdfMake.createPdf(reporteDocument).open();
}

module.exports.crearNota = crearNotaPDF;
module.exports.crearInventario = crearInventarioPDF;
module.exports.crearOrden = crearOrdenCompraPDF;
module.exports.crearReporte = crearReportePDF;

//Usar el modulo con require, ejemplo
//const pdf = require('ubicacion de este archivo');